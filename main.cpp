...
int main() {
    ...
    SHADERS::BonesShader bones_shader;
    ...
    Component::Render shadowmap_debug_rectangle;
    Debug::create_render_rectangle_top_right(shadowmap_debug_rectangle); 
    
	auto* _window = Window::get_window();
    ControllerCameraFunctions::CameraPosTemp new_cam;
    NewGui& new_gui = NewGui::get();
    GUI_USER_CODE::init(_window);
	...
	while (!Window::window_should_close()) {
		...
		if ((now - last_frame) >= fps_limit) {
			...
			GUISystem::new_editor_copy_over();
            rotation_copy_over();
			...
			auto [u_rad_fov, u_len, u_front, u_pos, u_col] = character_controller.light.get_uniform_variables_radfov_len_front_pos_col();
...

			{
				...
				glm::mat4 light_matrix = character_controller.light.get_light_matrix();
				unsigned int shadowmap_id = shadowmap.fbo.m_texture.id;

                // no uniform color
				glActiveTexture(GL_TEXTURE2);
				glBindTexture(GL_TEXTURE_2D, shadowmap_id);
				glUniform1i(glGetUniformLocation(flashlight_shader_new.shader_program, "shadowmap"), 2);
				...
				glActiveTexture(GL_TEXTURE2);
				glBindTexture(GL_TEXTURE_2D, shadowmap.fbo.m_texture.id);
				glUniform1i(glGetUniformLocation(flashlight_shader_normal_map.shader_program, "shadowmap"), 2);


				flashlight_shader_normal_map.uniform(glm_view, glm_proj, u_pos, u_front, u_rad_fov, u_len, u_col, light_matrix);
				...
				Renderer::representation_renderer_normalmap_bullet(&(registry.registry),
						flashlight_shader_normal_map.shader_program, flashlight_shader_normal_map.u_model);
				...
				glBindTexture(GL_TEXTURE_2D, shadowmap_id);
				glUniform1i(glGetUniformLocation(flashlight_shader_new.shader_program, "shadowmap"), 2);
				flashlight_shader_new.uniform(glm_view, glm_proj, u_pos, u_front, light_matrix);
				Renderer::bullet_temp_renderer(&(registry.registry),
						flashlight_shader_new.shader_program, flashlight_shader_new.u_model);

                bones_shader.bind();
                bones_shader.uniform(glm_view, glm_proj);
                Renderer::bones_render(&(registry.registry),            bones_shader);
                Renderer::bones_render(&(editor_registry.m_registry),   bones_shader);
			}
			...
			GUI_USER_CODE::new_frame();
			GUI_USER_CODE::update();
			GUI_USER_CODE::render();
